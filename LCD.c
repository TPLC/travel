...
sbit lcd_sid=P0^4;  /*接口定义:lcd_sclk就是LCD的sclk*/
sbit lcd_sclk=P0^5;   /*接口定义:lcd_sid就是LCD的sid sid就是SDA*/
sbit lcd_rs=P0^6;    /*接口定义:lcd_rs就是LCD的A0 数据命令选择端口*/
sbit lcd_reset=P0^7; /*接口定义:lcd_reset就是LCD的reset*/
sbit lcd_cs1=P1^0;   /*接口定义:lcd_cs1就是LCD的cs1 片选信号端*/

sbit Rom_CS=P1^1;    /*字库IC接口定义Rom_CS就是字库IC的CS#*/
sbit Rom_SCK=P1^2;   /*字库IC接口定义:Rom_SCK就是字库IC的SCK*/
sbit Rom_IN=P1^3;    /*字库IC接口定义:Rom_IN就是字库IC的SI*/
sbit Rom_OUT=P1^4;   /*字库IC接口定义:Rom_OUT就是字库IC的SO*/
...
void main(void)
{
	...
	/*    display_graphic_8x16(7,50,alphaa);
		display_GB2312_string(1,1,"GB2312简体字库");
		delay(2000);
		clear_screen();
	}*/
}
